...
class Curve():
    def __init__(self, x0, xf, v0, vf, tf, a0=0, af=0):
        ...
        self.a[3] = (20 * (xf - x0) - (8 * vf + 12 * v0) *
                     tf - (3 * a0 - af) * tf**2) / (2 * tf**3)
        self.a[4] = (30 * (x0 - xf) + (14 * vf + 16 * v0) *
                     tf + (3 * a0 - 2 * af) * tf**2) / (2 * tf**4)
        self.a[5] = (12 * (xf - x0) - 6 * (vf + v0) *
                     tf - (a0 - af) * tf**2) / (2 * tf**5)
        print(self.a)

    def calc(self, x):
        return self.a[0] + self.a[1] * x + self.a[2] * x**2 + \
            self.a[3] * x**3 + self.a[4] * x**4 + self.a[5] * x**5
...
class Planning():
    ...
    def plot(self):
        plt.plot(np.arange(len(self.cource)) *
                 self.dt, self.cource, marker='.')
